...
```yaml
---
- name: Golden VM 복제 후 개별 네트워크 설정 자동화
  hosts: all
  become: yes
  vars:
    # 인벤토리에서 정의한 변수를 사용 (예: hostname, static_ip)
  tasks:
    # 1. 호스트네임 변경
    - name: 시스템 호스트네임 변경
      hostname:
    ...
    - name: /etc/hosts 파일 내 호스트네임 업데이트
      lineinfile:
        path: /etc/hosts
        regexp: '^127.0.1.1'
        line: "127.0.1.1 {{ inventory_hostname_short }}"

    # 2. Netplan을 이용한 고정 IP 설정 (Ubuntu 표준)
    - name: Netplan 고정 IP 설정 적용
      copy:
