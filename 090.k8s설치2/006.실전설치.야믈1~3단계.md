  ...
  tasks:
    ...
    - name: 키링 저장용 디렉토리 생성 
      file: 
        ...
        state: directory 
        mode: '0755'
    - name: 기존에 잘못된 K8s 관련 설정 및 캐시 강제 삭제 (청소)
      shell: |
    ...
    - name: K8s 공식 GPG 키 다운로드 및 변환
      shell: |
        curl -fsSL https://pkgs.k8s.io/core:/stable:/v1.30/deb/Release.key | gpg --dearmor -o /etc/apt/keyrings/kubernetes-apt-keyring.gpg
      args:
        creates: /etc/apt/keyrings/kubernetes-apt-keyring.gpg

    - name: K8s 공식 소스 리스트 추가 (signed-by 적용)
      apt_repository: 
...
# 3-2단계: 클러스터 제어 권한 설정 (마스터 노드 전용)
    ...
    - name: admin.conf 파일을 사용자 kubeconfig로 복사
      copy:
        ...
        mode: '0600'
      ignore_errors: yes  # 워커 노드에서는 admin.conf가 없으므로 에러 무시 설정
