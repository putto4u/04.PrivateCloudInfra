...
```yaml
---
- name: Kubernetes OS 및 환경 최적화 (재실행 안전화 버전)
  hosts: all
  become: yes
  tasks:
    # 0단계: 기존 설정 확인 및 충돌 방지 (Clean Up)
    - name: 기존 쿠버네티스 설정 존재 여부 확인
      stat:
        path: /etc/kubernetes/kubelet.conf
      register: kube_config

    - name: 이전 클러스터 정보 초기화 (기존 설정이 있을 경우만 실행)
      shell: |
        kubeadm reset -f
        rm -rf /etc/cni/net.d
    ...
    - name: 스왑(Swap) 메모리 비활성화
      ...
      changed_when: "'switched off' in swap_result.stdout or swap_result.rc == 0"
    ...
    # 2단계: 컨테이너 런타임(Containerd) 설치 및 최적화
    - name: Containerd 설치
      apt:
        name: containerd
        state: present

    - name: Containerd 기본 설정 적용 (SystemdCgroup 활성화)
      shell: |
