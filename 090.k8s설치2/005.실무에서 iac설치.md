...
# 2. sed 명령어를 사용하여 'name: node' 부분을 현재 실제 호스트네임으로 치환
sudo sed -i "s/name: node/name: $(hostname)/g" kubeadm-init.yaml
# 3. 핵심 파라미터 자동 수정 (마스터 IP 및 Pod 네트워크 범위)
# 실행 전 마스터 노드의 IP 주소를 MASTER_IP 변수에 할당하세요.
MASTER_IP=$(hostname -I | awk '{print $1}')
sudo sed -i "s/advertiseAddress: 1.2.3.4/advertiseAddress: $MASTER_IP/g" kubeadm-init.yaml
sudo sed -i "s/kubernetesVersion: 1.30.0/kubernetesVersion: 1.30.14/g" kubeadm-init.yaml

# 4. Pod 네트워크(CIDR) 설정 추가 (Calico/Flannel 표준 범위)
# networking 섹션 아래에 podSubnet 설정을 삽입합니다.
sudo sed -i '/serviceSubnet: 10.96.0.0\/12/a \  podSubnet: 10.244.0.0/16' kubeadm-init.yaml

# 수정된 호스트네임과 버전 정보 확인
grep -E "name:|kubernetesVersion:|advertiseAddress:|podSubnet:" kubeadm-init.yaml
...
24시간이 지난경우 토큰 무효 됨. 재 발행하여 사용하여야 합니다.   
마스터노드에서 ```kubeadm token create --print-join-command ``` 실행후 나온 토큰을 메모해 놓으세요.
...
[수정] 효율적인 admin.conf 배포 및 설정 프로세스
불필요한 임시 복사 단계를 줄이고, 워커 노드에서도 즉시 사용 가능한 경로로 파일을 배치합니다.

1. Master Node: 전송용 권한 부여 (단일 공정)
파일을 다른 곳으로 복사하지 않고, 현재 위치에서 소유권만 변경하여 전송 준비를 마칩니다. (또는 전송 직후 권한을 환원하여 보안을 유지합니다.)

Bash
# 관리자 설정 파일을 일반 사용자가 읽을 수 있도록 임시로 권한 변경
# 복사 과정 없이 원본의 접근 권한만 조정하여 scp 준비
```
sudo chmod 644 /etc/kubernetes/admin.conf
```
2. SCP 전송: 타겟 경로 최적화
워커 노드에 미리 디렉토리를 만들 필요 없이, 전송 시점에 경로를 명시하여 한 번에 보냅니다.

Bash
# 워커 노드의 .kube 디렉토리가 없다면 원격에서 생성 후 바로 전송
```
ssh master@192.168.0.201 "mkdir -p ~/.kube"
scp /etc/kubernetes/admin.conf master@192.168.0.201:~/.kube/config
```

# 전송 완료 후 마스터 노드 원본 권한 복구 (보안 유지)
```sudo chmod 600 /etc/kubernetes/admin.conf```
3. Worker Node: 권한 최적화
워커 노드에서는 이미 ~/.kube/config 위치로 파일이 들어와 있으므로, 소유권과 권한만 확인하면 즉시 kubectl 사용이 가능합니다.

Bash
# 워커 노드에서 실행: 본인 소유 확인 및 권한 제한
