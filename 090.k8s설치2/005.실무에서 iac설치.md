...
> [!NOTE]
> **AWS 환경 참고:** > AWS CloudShell이나 이미 구성된 머신 이미지(AMI)에는 기본적으로 이 설정이 포함되어 있을 수 있습니다. 하지만 직접 설치한 Ubuntu 등의 환경에서는 위 과정을 반드시 거쳐야 합니다. 이 설정은 시스템 리소스를 거의 차지하지 않으므로 추가적인 컴퓨팅 비용은 발생하지 않습니다.

---

Next Step: YAML 매니페스트 구조와 API 리소스 이해

---

명령어 자동 완성은 단순한 편의 기능을 넘어 운영 실수를 방지하는 핵심 설정입니다. 특히 별칭(`k`) 설정까지 완료하시면 훨씬 쾌적하게 클러스터를 관리하실 수 있습니다.
