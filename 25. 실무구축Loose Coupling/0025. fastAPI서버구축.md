...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: fast-container
        # [Image] 기존 Python 환경 유지를 위해 동일 이미지 사용 (혹은 python:3.9-slim 권장)
        image: tiangolo/uvicorn-gunicorn-fastapi:python3.9-slim
        
        # [수정됨] 컨테이너 실행 명령 변경
        # 기존 uvicorn(seller:app) 실행 로직을 제거하고,
        # 파이썬 내장 http.server 모듈을 사용하여 정적 웹 서버로 전환
        command: ["python"]
        
        # [수정됨] 실행 인자 (Arguments)
        # -m http.server : 간단한 HTTP 서버 실행
        # 80             : 80번 포트로 리슨
        # --directory    : 서빙할 루트 디렉터리 지정 (/data)
        args: 
          - "-m"
          - "http.server"
          - "80"
        ...
        # [Volume] 컨테이너 내부 데이터 경로 (/data에 index.html 위치 예상)
        volumeMounts:
        - name: fast-vol
          mountPath: /data

      # [Volume] 호스트 경로 연결 (워커 노드의 해당 경로에 index.html 파일 필수)
      volumes:
      - name: fast-vol
        hostPath:
          path: /home/master/k8s/3tier/fastdata
