...
앞서 소개한 `kube-prometheus-stack` 헬름 차트는 단순한 프로메테우스 서버뿐만 아니라, 매트릭을 생성하고 수집하는 **모든 필수 에이전트(Exporter)를 포함**하고 있습니다. 따라서 별도로 매트릭 수집기를 설치할 필요가 없습니다.

본 장에서는 이 '올인원(All-in-One)' 패키지가 자동으로 설치해 주는 수집기들의 종류와 그 역할에 대해 상세히 다룹니다.
