...
# Chapter 4. Prometheus 설치 후 검증 및 외부 접근 구성

Prometheus의 'All-in-One' 설치(Helm Chart 기반) 직후에는 보안상의 이유로 클러스터 내부에서만 접근 가능한 상태로 배포됩니다. 본 챕터에서는 설치된 구성 요소들의 상태를 검증하고, 로컬 VM 환경에서 대시보드에 접근하기 위해 Service Type을 `NodePort`로 변경하는 절차를 다룹니다.

---

### 1. 핵심 구성 요소 상태 검증 (Health Check)

서비스를 노출하기 전에 핵심 컴포넌트인 Prometheus Server 파드가 정상적으로 구동 중인지 확인해야 합니다. 파드가 생성되지 않았다면 서비스 연결 자체가 불가능합니다.

**명령어:**

```bash
# Monitoring 네임스페이스 내의 모든 파드 조회
...
1. **Prometheus Server:** `prometheus-stack-kube-prom-prometheus-0` (또는 유사 명칭) 파드가 반드시 존재해야 하며, `STATUS`가 `Running`이어야 합니다.
2. **Operator:** `prometheus-operator` 파드가 `Running` 상태여야 설정 변경이 반영됩니다.
3. **StatefulSet:** 만약 파드가 보이지 않는다면 StatefulSet 컨트롤러 상태를 확인합니다.
```bash
...
### 2. Service 식별 및 타입 변경 (NodePort)

기본 설정된 `ClusterIP`는 클러스터 내부 통신 전용입니다. 온프레미스 또는 VM 환경에서는 워커 노드의 IP와 포트를 통해 직접 접근하는 `NodePort` 방식을 사용합니다.

**Step 1: 대상 서비스 명칭 식별**
설치된 헬름 차트의 릴리스 이름에 따라 서비스 명칭이 달라질 수 있습니다. 정확한 서비스 이름을 먼저 찾습니다.
...
* **찾아야 할 대상:** `9090` 포트를 사용하는 Prometheus Server 서비스 (예: `prometheus-stack-kube-prom-prometheus`)

**Step 2: Service Type 변경 (Patch)**
식별된 서비스의 타입을 `ClusterIP`에서 `NodePort`로 실시간 변경합니다.

```bash
# 서비스 명칭은 위에서 확인한 실제 이름으로 대치 (예시: prometheus-stack-kube-prom-prometheus)
...
NodePort로 변경하면 30000~32767 범위 내에서 무작위 포트가 할당됩니다.
...
prometheus-stack-kube-prom-prometheus   NodePort   10.103.209.177   <none>        9090:31234/TCP   45m

```

* **9090:** 컨테이너 내부 포트
* **31234:** 외부(Host)에서 접속할 포트 (이 번호를 기록합니다)

---

### 4. 방화벽 설정 및 브라우저 접속

마지막으로 VM의 네트워크 보안 정책을 점검하고 접속을 시도합니다.

**Step 1: 방화벽 오픈 (필요 시)**
VM 자체 방화벽(UFW, Firewalld)이나 상단 네트워크 방화벽에서 위에서 확인한 NodePort(예: 31234)를 허용해야 합니다.

**Step 2: 웹 콘솔 접근**

* **URL:** `http://<VM_Node_IP>:<Assigned_NodePort>`
* **검증:** Prometheus UI의 상단 메뉴 `Status` -> `Targets`에 진입하여 `node-exporter` 등의 타겟들이 `UP` 상태인지 확인합니다.

---

Next Step: Prometheus 데이터 영속성을 위한 StorageClass 및 PV/PVC 연결 설정
