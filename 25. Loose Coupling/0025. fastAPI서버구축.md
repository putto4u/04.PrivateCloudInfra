  ...
  template:
    ...
    spec:
      ...
      - name: fastapi-container
        image: python:3.11-slim  # 가벼운 이미지 사용
        workingDir: /app
        command: ["/bin/sh", "-c"]
        args:
          - |
            pip install fastapi uvicorn pydantic  # 필요한 패키지 설치
            # main.py가 없을 경우를 대비한 기본 파일 생성 (실습용)
            if [ ! -f main.py ]; then
              echo "from fastapi import FastAPI; app = FastAPI(); @app.get('/')\ndef root(): return {'message': 'FastAPI on K8s Success!'}" > main.py
            fi
      ...
      - name: fastapi-data
        persistentVolumeClaim:
          claimName: ng-static-pvc  # 기존 PVC 재사용 (데이터 공유 테스트)
---
apiVersion: v1
kind: Service
metadata:
...
spec:
  ...
  ports:
    - protocol: TCP
      port: 8000
      targetPort: 8000
      nodePort: 30081  # 엔진엑스(30080)와 충돌하지 않도록 설정
