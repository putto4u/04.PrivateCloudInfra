...
2. 운영 절차 가이드 (명령어 요약)
   ...
   NFS 마운트 버전 실행   ```kubectl apply -k .``` 을 통해 볼륨 설정 병합 배포(파일명이 예약된 고정명임)
   **Kustomizaion의 경우 파일명이 예약어로 정해져 있습니다. kustomization.yml 

   NFS 해제 (원복)    ```kubectl apply -f fweb.yml```    원본 파일로 덮어씌워 볼륨 제거 및 초기화

## bweb  PVC
백엔드 경우, 패치야믈을 만들고 Kustomization으로 연결하지 않고, 커스터마이징 파일내에 pvc를 넣은 사례입니다. 
```
apiVersion: kustomize.config.k8s.io/v1beta1
kind: Kustomization
...
# 2. 최신 Strategic Merge Patch 형식
patches:
  - target:
      kind: Deployment
      name: bweb-deployment
    patch: |
      apiVersion: apps/v1
      kind: Deployment
      metadata:
...
앞서 고정된 파일명으로 작업을 하게 될 경우. 두번째 작업에서 파일명이 겹치게 됩니다. 
이러면 두개의 파일명을 유지할 수 없습니다. 
그래서 아래와 같이 파일을 정해진 파일명으로 복사후 작업을 한후에 제거하는 명령으로 대치하였습니다. 
아래 명령은 좋은 사례입니다. 

```
cp bweb_kustom.yml kustomization.yaml && (kubectl apply -k . ; rm kustomization.yaml)
```
