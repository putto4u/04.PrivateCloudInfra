...
k describe deploy dp01

--------
**Rollback
k rollout undo deploy dp01    
...
디플로이먼트 롤백은 **'쿠버네티스 오브젝트 설정'**을 되돌리는 것입니다. 만약 애플리케이션 내부에서 **데이터베이스 스키마를 변경**했거나, 외부 스토리지의 데이터를 삭제했다면 롤백만으로는 완벽한 복구가 불가능합니다. 따라서 데이터베이스 마이그레이션이 포함된 배포는 반드시 별도의 백업 전략이 병행되어야 합니다.
