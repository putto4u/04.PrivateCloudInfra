...
kubeadm certs check-expiration   # 쿠버네티스 인증서 상태 확인
  Missing!  # 최초 미싱 상태
kubeadm config images list    # kubeadm이 사용할 수 있는 이미지 리스트
```

```
sudo -i   # k8s 명령줄 설치시
# kubeadm config images pull   #  k8s 설치에 필요한 이미지를 다운로드
# kubeadm config images pull  --cri-socket  /run/containerd/containerd.sock   # CRI 소켓  CRI를 containerd로 설정하여 이미지를 받음
