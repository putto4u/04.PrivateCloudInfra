쿠버네티스 운영 및 교육 시 가장 빈번하게 사용되는 핵심 명령어들을 기능별로 분류하여 정리해 드립니다. 강사님의 교재에 바로 활용하실 수 있도록 실전 예제와 함께 구성했습니다.

---

## 쿠버네티스 핵심 명령어 가이드 (Cheat Sheet)

### 1. 리소스 생성 및 관리 (Management)

| 명령어 | 기능 설명 | 주요 옵션 | 실전 예제 |
| --- | --- | --- | --- |
| **apply** | 구성 파일을 기반으로 리소스를 생성하거나 상태를 업데이트 (선언적 방식) | `-f` (파일/디렉토리), `-R` (재귀적 적용) | `kubectl apply -f pod.yaml` |
| **create** | 새로운 리소스를 생성 (명령형 방식, 기존 존재 시 에러) | `--dry-run=client -o yaml` (템플릿 생성) | `kubectl create deployment web --image=nginx` |
| **run** | 단일 Pod를 생성하여 실행 | `--image`, `--port`, `--env` | `kubectl run tmp-shell --image=busybox -it -- rm` |
| **delete** | 리소스를 삭제 | `--force`, `--grace-period=0` (즉시 삭제) | `kubectl delete pod my-pod --force` |

### 2. 상태 확인 및 조회 (Inspection)

| 명령어 | 기능 설명 | 주요 옵션 | 실전 예제 |
| --- | --- | --- | --- |
| **get** | 리소스 목록을 출력 | `-o wide` (상세정보), `-n` (네임스페이스), `-w` (감시) | `kubectl get pods -n kube-system -o wide` |
| **describe** | 특정 리소스의 상세 상태와 이벤트 이력을 확인 (디버깅 필수) | (객체 종류 및 이름 필요) | `kubectl describe node k8s-master` |
| **logs** | 컨테이너의 표준 출력 로그를 확인 | `-f` (실시간 팔로우), `--tail` (마지막 N줄), `-c` (멀티컨테이너 지정) | `kubectl logs -f my-web-pod --tail 20` |
| **explain** | 리소스별 YAML 필드 정의 및 설명 확인 | (필드 경로) | `kubectl explain pod.spec.containers` |

### 3. 디버깅 및 상호작용 (Troubleshooting)

| 명령어 | 기능 설명 | 주요 옵션 | 실전 예제 |
| --- | --- | --- | --- |
| **exec** | 실행 중인 컨테이너 내부에서 명령어를 실행 | `-it` (대화형 터미널) | `kubectl exec -it my-pod -- /bin/bash` |
| **port-forward** | 로컬 포트를 특정 Pod/Service 포트로 터널링 (외부 노출 전 테스트용) | `--address 0.0.0.0` (외부 접속 허용) | `kubectl port-forward svc/web-svc 8080:80` |
| **cp** | 로컬과 컨테이너 간에 파일/디렉토리 복사 | (소스 및 대상 경로) | `kubectl cp ./config.conf my-pod:/etc/config.conf` |
| **top** | 노드나 Pod의 CPU/메모리 리소스 사용량 확인 | `--containers` | `kubectl top pod --all-namespaces` |

### 4. 노드 및 클러스터 관리 (Cluster Ops)

| 명령어 | 기능 설명 | 주요 옵션 | 실전 예제 |
| --- | --- | --- | --- |
| **cordon** | 해당 노드에 새로운 Pod가 스케줄링되지 않도록 설정 | (노드 이름) | `kubectl cordon k8s-worker1` |
| **drain** | 노드 점검을 위해 실행 중인 Pod를 다른 노드로 퇴거 | `--ignore-daemonsets`, `--delete-emptydir-data` | `kubectl drain k8s-worker1` |
| **label** | 리소스에 레이블을 추가하거나 수정 | `--overwrite` | `kubectl label nodes worker1 disk=ssd` |
| **config** | 쿠버네티스 접속 설정(kubeconfig) 관리 | `view`, `use-context`, `set-context` | `kubectl config use-context my-cluster` |
