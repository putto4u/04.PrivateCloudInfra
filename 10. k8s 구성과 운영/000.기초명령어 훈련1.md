...
## 쿠버네티스 실전 명령어 가이드 (Cheat Sheet)

### 1. 리소스 생성 및 관리 (Management)

| 실전 예제 | 명령어 | 기능 설명 | 주요 옵션 |
| --- | --- | --- | --- |
| `k apply -f pod.yaml` | **apply** | 구성 파일을 기반으로 상태를 업데이트 (선언적 방식) | `-f` (파일), `-R` (하위 디렉토리 포함) |
| `k create deploy web --image=nginx` | **create** | 새로운 리소스를 즉시 생성 (기존 존재 시 에러) | `--dry-run=client -o yaml` (템플릿 추출) |
| `k run debug --image=busybox -it -- rm` | **run** | 단일 Pod를 생성하고 대화형으로 실행 후 삭제 | `--image`, `-it` (터미널 접속) |
| `k delete pod my-pod --force` | **delete** | 리소스를 삭제 (강제 종료 시 사용) | `--grace-period=0` (즉시 삭제) |

### 2. 상태 확인 및 조회 (Inspection)

| 실전 예제 | 명령어 | 기능 설명 | 주요 옵션 |
| --- | --- | --- | --- |
| `k get pods -n kube-system -o wide` | **get** | 리소스 목록과 상세 정보(IP, Node) 출력 | `-o wide`, `-w` (실시간 감시), `-A` (전체) |
| `k describe node k8s-master` | **describe** | 특정 리소스의 상세 상태와 이벤트 이력을 확인 | (객체 종류 및 이름 필수) |
| `k logs -f my-pod --tail 20` | **logs** | 컨테이너의 로그를 실시간으로 추적 | `-f` (팔로우), `-c` (멀티컨테이너 지정) |
| `k explain pod.spec.containers` | **explain** | YAML 정의서의 각 필드 의미와 구조 확인 | (필드 경로) |

### 3. 디버깅 및 상호작용 (Troubleshooting)

| 실전 예제 | 명령어 | 기능 설명 | 주요 옵션 |
| --- | --- | --- | --- |
| `k exec -it my-pod -- /bin/bash` | **exec** | 실행 중인 컨테이너 내부 쉘에 접속 | `-it` (대화형 터미널) |
| `k port-forward svc/web 8080:80` | **port-forward** | 로컬 포트를 서비스/포드 포트로 터널링 | `--address 0.0.0.0` (외부 바인딩) |
| `k cp ./app.conf my-pod:/etc/` | **cp** | 로컬 PC와 컨테이너 간 파일 복사 | (소스 경로, 대상 경로) |
| `k top pod --all-namespaces` | **top** | 실제 리소스(CPU, Mem) 사용량 확인 | `--containers`, `--sort-by` |

### 4. 클러스터 운영 및 관리 (Cluster Ops)

| 실전 예제 | 명령어 | 기능 설명 | 주요 옵션 |
| --- | --- | --- | --- |
| `k cordon worker-1` | **cordon** | 해당 노드에 신규 포드 배정 금지 | (노드 명칭) |
| `k drain worker-1 --ignore-daemonsets` | **drain** | 유지보수를 위해 노드 내 포드들을 타 노드로 이동 | `--delete-emptydir-data` |
| `k label nodes worker-1 disk=ssd` | **label** | 리소스에 메타데이터(레이블) 부여 및 수정 | `--overwrite` |
| `k config use-context prod-cluster` | **config** | 여러 클러스터 중 작업 대상을 전환 | `view`, `set-context` |

---

### 💡 실전 팁: 전문가의 명령형(Imperative) 활용법

강의 시 수강생들에게 **"YAML을 처음부터 짜지 마라"**고 강조해 주세요. `kubectl create ... --dry-run=client -o yaml > base.yaml` 명령을 통해 기본 뼈대를 먼저 뽑아낸 뒤, 필요한 부분만 수정하는 것이 실무에서 오타를 줄이고 속도를 높이는 핵심 비결입니다.

---

### ⚠️ 비용 및 유료 전환 주의사항

* **AWS Metric Server**: `kubectl top` 명령어를 사용하려면 클러스터 내에 Metrics Server가 구동되어야 합니다. 이는 미세한 CPU/메모리 리소스를 점유하며, AWS EKS 환경에서 **Fargate**를 사용할 경우 리소스 점유량에 따른 비용이 산정됩니다.
* **EBS/EFS 볼륨**: `delete` 명령어로 Pod를 지워도, `PersistentVolumeClaim(PVC)`이 삭제되지 않으면 연결된 **AWS EBS 볼륨 비용**은 계속 청구됩니다. 리소스 삭제 시 볼륨 정책을 반드시 확인하십시오.

---

**Next Step: kubectl 가상 별칭(Alias) 설정 및 자동 완성(Completion) 적용**

Next Step: **강의 교재용 명령어 퀴즈** | **실무 장애 조치 시나리오**

### 💡 실전 팁: 전문가의 효율적인 활용법

* **Alias 설정**: `alias k='kubectl'`를 `.bashrc`에 등록하여 입력 시간을 단축하세요.
* **Auto-completion**: `source <(kubectl completion bash)` 명령어를 통해 탭(Tab) 키로 명령어를 자동 완성하면 오타를 획기적으로 줄일 수 있습니다.
* **Dry-run 활용**: 실제 반영 전 `kubectl apply -f pod.yaml --dry-run=server`를 실행하여 설정값의 유효성을 미리 검증하는 습관을 들이는 것이 좋습니다.
