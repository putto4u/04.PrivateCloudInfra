...
---
쿠버네티스의 패키지 매니저인 **헬름(Helm)**의 개념과 설치 방법을 정리한 강의 교재입니다.

---

## 헬름(Helm) 설치 및 기본 개념

쿠버네티스 환경에서 애플리케이션을 배포할 때, 수많은 YAML 파일(Deployment, Service, Ingress 등)을 일일이 관리하는 것은 매우 번거로운 작업입니다. **헬름(Helm)**은 이러한 복잡한 리소스들을 하나의 패키지 단위로 묶어 관리할 수 있게 해주는 '쿠버네티스 전용 패키지 매니저'입니다.

### 1. 헬름(Helm)이란?

* **패키지 매니저:** 리눅스의 `apt`나 `yum`, 파이썬의 `pip`와 유사한 역할을 합니다.
* **차트(Chart):** 헬름에서 사용하는 패키지 단위를 의미하며, 쿠버네티스 리소스를 정의한 템플릿 파일들의 집합입니다.
* **릴리스(Release):** 차트가 클러스터에 설치되어 실제로 구동 중인 인스턴스를 의미합니다.

### 2. 헬름 설치 방법 (Linux/macOS)

헬름은 클라이언트 도구이므로 `kubectl`이 설정된 로컬 PC나 마스터 PC에 바이너리 파일을 설치하면 됩니다.

#### 방법 1: 공식 설치 스크립트 이용 (가장 권장)

공식에서 제공하는 스크립트를 통해 시스템 환경에 맞는 최신 버전을 자동으로 설치합니다.
...
각 운영체제별 패키지 관리 도구를 사용할 수도 있습니다.
...
설치가 완료되면 버전을 확인하고, 전 세계 사용자들이 공유하는 공식 차트 저장소(Artifact Hub 등)를 사용하기 위해 리포지토리를 추가해야 합니다.

```bash
# 설치 버전 확인
helm version

# 공식 리포지토리(bitnami) 추가 예시
helm repo add bitnami https://charts.bitnami.com/bitnami

# 리포지토리 목록 업데이트 (최신 정보 동기화)
helm repo update

```

### 4. 헬름 주요 명령어 (Cheat Sheet)

| 기능 | 명령어 |
| --- | --- |
| **차트 검색** | `helm search repo <키워드>` |
| **차트 설치** | `helm install <이름> <차트명>` |
| **설치 목록 확인** | `helm list` |
| **설치 상태 확인** | `helm status <이름>` |
| **설치 삭제** | `helm uninstall <이름>` |

---

### 5. 실무 현장 사례 (Case Study)

#### 사례 1: 환경별 배포 자동화 (Dev/Prod 격리)

**상황:** 개발 서버와 운영 서버에 동일한 엔진엑스를 배포해야 하지만, 서버 성능(CPU/Mem)과 복제본(Replicas) 개수가 달라야 함.

* **조치:** 헬름의 `values.yaml` 파일을 환경별로 분리하여 관리.
* **명령어:** ```bash
# 개발 환경 배포


helm install my-web bitnami/nginx -f values-dev.yaml
```
...
#### 사례 2: 애플리케이션 롤백 (Rollback)

**상황:** 새로 배포한 애플리케이션에 치명적인 버그가 발견되어 즉시 이전 버전으로 되돌려야 함.

* **조치:** 헬름의 히스토리 기능을 통해 이전 릴리스 번호로 원클릭 복구.
* **명령어:**
```bash
# 배포 이력 확인
helm history my-web
...
* 헬름은 단순한 설치 도구를 넘어 **버전 관리**와 **템플릿화**를 가능하게 합니다.
* 실무에서는 직접 차트를 만들기보다 `Artifact Hub`에서 검증된 차트를 가져와 `values.yaml`만 수정하여 사용하는 경우가 많습니다.
* **AWS 과금 주의:** 헬름으로 로드밸런서가 포함된 차트(예: ingress-nginx)를 설치하면 즉시 AWS ELB 비용이 발생하므로, 실습 후에는 반드시 `helm uninstall`로 리소스를 삭제해야 합니다.

---

**Next Step:**
**헬름을 이용한 Nginx Ingress Controller 설치 및 외부 도메인 연결**
